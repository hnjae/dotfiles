...
function M:peek(job)
  ...
  ya.sleep(
    math.max(
      0,
      (rt and rt.preview or PREVIEW).image_delay / 1000 + start - os.clock()
    )
  )
  ...
  if output then
    ...
    for str in output:gsub("\n+$", ""):gmatch("[^\n]*") do
      ...
      if label then
        if not skip_labels[label] then
          line = ui.Line({
            ...
            ui.Span(value):style(
              (th and th.spot and th.spot.tbl_col) or ui.Style():fg("blue")
            ),
          })
        end
      elseif str ~= "General" then
        line = ui.Line({
          ui.Span(str)
            :style((th and th.spot and th.spot.title) or ui.Style():fg("green")),
        })
      end

      if line then
        local line_height =
          math.max(1, is_wrap and math.ceil(line:width() / max_width) or 1)
  ...
  if
    ...
    and (not is_video or (is_video and job.skip >= 90))
  then
...
function M:preload(job)
  local cache_img_url_no_skip = ya.file_cache({ file = job.file, skip = 0 })
  local cache_img_url_no_skip_cha = cache_img_url_no_skip
    and fs.cha(cache_img_url_no_skip)
  ...
  if job.mime and string.find(job.mime, "^video/") then
    ...
    local cache_img_status, video_preload_err =
      video:preload({ file = job.file, skip = math.min(90, job.skip) })
  ...
  if
    cache_img_url_no_skip
    and (not cache_img_url_no_skip_cha or cache_img_url_no_skip_cha.len <= 0)
  then
    -- audio
    if job.mime and string.find(job.mime, "^audio/") then
      local qv = 31
        - math.floor((rt and rt.preview or PREVIEW).image_quality * 0.3)
    ...
    elseif job.mime and string.find(job.mime, "^image/") then
      ...
      local image = magick_image_mimes[mime]
          and ((mime == "svg+xml" and svg_plugin_ok) and svg_plugin or require(
            "magick"
          ))
        or require("image")
      ...
      if not cache_img_status and image_preload_err then
        err_msg = err_msg
          .. "Failed to cache image , check cache folder permissions\n"
      end
    end
  end
  ...
  local cmd = "mediainfo"
  local output, err =
    Command(cmd):args({ tostring(job.file.url) }):stdout(Command.PIPED):output()
  if err then
    err_msg = err_msg
      .. string.format(
        "Failed to start `%s`, Do you have `%s` installed?\n",
  ...
  return fs.write(
    cache_mediainfo_url,
    (err_msg ~= "" and "Error: " .. err_msg or "")
      .. (output and output.stdout or "")
  )
end
