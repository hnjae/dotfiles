local target_paths = ya.sync(function()
  local paths = {}
  ...
  if hovered then
    paths[1] = tostring(Url(hovered.url))
  end
...
local function count_label(count)
  return string.format("%d %s", count, count == 1 and "path" or "paths")
end
...
return {
  entry = function()
    ...
    local ok, err = copy_with_osc52(table.concat(paths, "\n"))
    if not ok then
      notify("error", err, 7)
      return
    end

    notify("info", string.format("Copied %s with OSC 52.", count_label(#paths)), 4)
  end,
}
