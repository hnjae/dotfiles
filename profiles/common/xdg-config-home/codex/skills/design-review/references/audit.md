...
- Require at least three investigation subagents, even when work must run in batches. If the environment cannot start or reuse three, stop before investigation without creating or replacing `DESIGN_REVIEW.md`, report the limitation, and request user direction.
- Mark `Review completion: Complete` only when every non-excluded coverage assignment and assigned authoritative document is inspected and every track has one current final report. Otherwise mark it `Partial`.
...
1. Assign each repository root an immutable `RNN` ID in discovery order.
2. Create an authority map listing every `docs/spec/**` and `docs/architecture/**` file or verified absence, missing-set eligibility evidence, cross-cutting provisions, and provisions initially relevant to Tracks 2–6.
3. Create an implementation coverage map of every top-level first-party component and every material entry point, contract, persistence boundary, concurrent path, resource lifecycle, and test boundary.
4. Define each non-excluded coverage assignment as one tuple `(root, cohesive component or boundary, track)`. Assign every tuple its own immutable `CU-NNN` ID. When the same component is relevant to two tracks, create two CU IDs.
5. Give each excluded inventory entry its own CU ID with track `Excluded` and a concrete exclusion reason.
6. Never reuse or renumber root or coverage IDs. Assign the next unused ID to newly discovered scope.
...
- Assign every CU and relevant authoritative document before delegation. Give each owner its track, authority-map entries, CU IDs, and named entry points and boundaries.
...
- Completed, partially inspected, unable-to-verify, and pending CU IDs and documents.
- Provisional observation labels, titles, decisive evidence, and blockers.
- Reserved canonical IDs from any superseded report.
- Next unused observation and candidate sequences.
- Requested follow-up scope.

The main agent may send `Finalize Track <number>` only after every assigned CU and document has a recorded status and no follow-up or transfer remains. Only then may the reporting owner issue canonical `T<track>-F<sequence>` candidate IDs and return one current final report.
...
- ID, title, finding type, authority status, priority, confidence, and assumptions.
...
A supported non-P0 concern omitted only because of the cap receives no candidate ID. Record it as a cap omission on every directly relevant checklist entry so the main agent can decide whether to request promotion.
...
- `Cap omissions`: zero or more entries containing priority, confidence, short title, decisive evidence, and comparative reason.
...
When a follow-up promotes a cap omission, replace that omission with its new candidate ID and record `Resolved cap omission — promoted as <candidate ID>` in the allocation audit history.
...
For every root record root ID, path, branch, start and final HEAD, start and final porcelain state, included local changes, and drift handling. Exclude `DESIGN_REVIEW.md` from all snapshots.

### Coverage and allocation

Include:

- An authority-map summary with reviewed paths, verified absence, missing-set eligibility, and material provisions.
...
- One implementation coverage row per CU ID with root, component or boundary, paths or entry points, track, focus, final inspection status, and exclusion or limitation.
...
- Material allocation changes, resolved cap-omission history, inventory uncertainty, unavailable runtime or organizational evidence, and repository drift.
...
- `Uncertain`: materially important but requiring additional evidence and appearing exactly once in open questions.
...
After writing `DESIGN_REVIEW.md`, run all checks below, correct repository-verifiable failures, and rerun the complete list. If a check cannot be completed, mark integrity `Not fully validated`, record the exact gap, and do not claim it passed.
...
6. Every RNN reference resolves to one root snapshot. Reverify every final or uncertain evidence item against the final snapshot.
7. Every CU ID appears in exactly one coverage-map row and represents one root-component-track tuple or one excluded entry. Every current track checklist accounts for its assigned CUs and documents with all four independent checklist fields.
8. Coverage-map, track-report, and aggregation statuses agree. Any partial or unavailable non-excluded scope or missing current track report requires `Review completion: Partial`.
9. Preserve every material exclusion, allocation change, owner failure, unresolved cap omission, partial inspection, and unavailable boundary under Coverage and Limitations. Do not present resolved cap omissions as current limitations.
10. Capture repository state once more after validation. If relevant state changed, update the snapshot, reverify affected evidence, and rerun validation once. If drift persists, move affected claims to open questions, mark completion Partial and integrity Not fully validated, then repair structural references without starting an unbounded snapshot loop.

Do not commit `DESIGN_REVIEW.md`.
