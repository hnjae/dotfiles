...
5. Record the device and inode or equivalent stable file identity and the hard-link count for every regular file, and group inventoried paths that share an identity. Never write in place to a file with multiple hard links. Treat every such path as an edit limitation, and treat a link count greater than the matching paths inventoried under the allowed roots as evidence of an unaccounted alias. Edit a hard-linked path only when the user explicitly authorizes breaking its hard-link relationship and an available verified replace-with-new-file operation changes only that allowed-root path without writing the shared inode; otherwise leave it unchanged and report the unresolved finding.
6. Except for the private working-state directory allowed below, list, read, search, create, and edit project entries only within the two allowed roots. Do not inspect source code, tests, package metadata, Git metadata or history, other documentation, or material outside those roots. Preserve file paths because inbound references outside the closed scope cannot be checked. Treat deleting, moving, or renaming a file as a separate path-changing operation that requires explicit user authorization clearly encompassing that operation; general repair authorization does not suffice. Disclose every authorized path change and that external inbound references remain unverified.
7. Immediately before every path-addressed project content read or write, repeat no-follow metadata checks for the target and its parents, revalidate containment under one allowed root, and compare the target with the current accepted snapshot. Metadata operations and no-follow content reads used solely to compute a candidate snapshot digest are exempt from the snapshot comparison but must be bounded to the allowed roots, verify identity, hard-link count, size, and modification time before and after the read, and not supply review evidence until the complete candidate manifest is stable and explicitly promoted. For a new destination, validate its parent and unresolved destination path. Reject and report a content target whose type, containment, identity, or hard-link count differs from the current accepted snapshot. Use recursive readers, search tools, or validators only when their no-follow behavior is known and their traversal is explicitly bounded to the allowed roots.
8. Treat path preflight as non-atomic. Permit writes only through a kernel-enforced descriptor-rooted operation that rejects symlinks and resolution outside an already-open allowed root, or an equivalently verified operation with the same guarantees. If no such write operation is available, remain review-only for the affected files and report the edit limitation under `Unresolved Findings`; never use a stable-tree assumption to authorize a write. For reads when descriptor-rooted access is unavailable, bracket each phase with the stable snapshot checks below, do not claim atomic containment, disclose the non-contractual stable-tree assumption, and stop when there is evidence that paths are mutating concurrently or snapshot drift repeats.
9. Do not create review notes, inventories, backups, or temporary artifacts in the repository. Keep working state in the conversation and tool results by default. When the corpus is too large for reliable in-context state, create one private temporary directory outside the project with a secure temporary-directory facility, restrict access to the current user, and store only the derived inventory, coverage ledger, contract facts, slice returns, and review decisions needed by this workflow. Record its exact path, never copy source documents into it, remove exactly that directory immediately before the final response, and disclose a cleanup failure.
...
Before first-pass reading, record the initial accepted read snapshot containing every relative path and entry type plus device and inode or equivalent stable identity, hard-link count, size, modification time, and a content digest for every readable regular file. Compute each digest through a no-follow read and confirm that identity, hard-link count, size, and modification time remain unchanged across that read. Treat the initial read snapshot, an explicitly promoted reconciliation baseline, or the reviewed snapshot as the current accepted snapshot only during the phase it governs.
...
Re-inventory and compare the complete manifest after the read-only first pass, immediately before the first edit, after the last edit, and immediately before the final report. When the post-first-pass manifest is unchanged, promote it to the current reviewed snapshot. Confirm that each existing file's current digest matches that reviewed snapshot immediately before editing it. Every addition, deletion, type change, identity change, hard-link-count change, or content change invalidates the affected coverage regardless of whether its cause is known. Stop writes and obtain a new stable no-follow manifest, then promote it only as the read-only reconciliation baseline. Against that baseline, read every new or changed supported entry, update removed entries, rebuild the affected subject and global model, and repeat the cross-subject checks. If a complete manifest comparison remains unchanged after reconciliation, promote that manifest to the current reviewed snapshot and re-enable only otherwise authorized writes. Record a known cause only for attribution and reporting. If drift repeats or cannot be reconciled without overwriting concurrent work, stop and report the limitation.
...
Under `Unresolved Findings`, report every verified material defect left unfixed because it is outside the authorized repair scope, requires separate path-change or hard-link authorization, cannot be edited or validated safely, or is blocked by a coverage or operational limitation. Identify the affected file and heading and the precise blocker. Reserve `Open Questions` for contract decisions the allowed evidence does not support.

For every material item, identify the affected file and heading. Under `Verification`, also state the number of entries in the final stable manifest and the number of fully reviewed files; distinguish files reviewed directly by the primary agent from subject slices first-pass reviewed by subagents; identify each delegated slice; list every skipped or partially reviewed entry and every unresolved normative external dependency; state when coverage limitations restrict conclusions to fully reviewed supported files and prohibit a global no-additional-findings claim; disclose any authorized deletion, move, rename, or hard-link separation whose external inbound references could not be checked; and report whether a private working-state directory was removed successfully.
