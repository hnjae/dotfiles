...
M.setup = function()
  ...
  vim.api.nvim_create_autocmd({ "BufReadPost" }, { group = au_id, callback = set_colorcolumn })
  vim.api.nvim_create_autocmd({ "BufNewFile" }, { group = au_id, callback = set_colorcolumn })
  vim.api.nvim_create_autocmd({ "OptionSet" }, { pattern = { "textwidth" }, group = au_id, callback = set_colorcolumn })
end
